package com.elevysi.site.zuul.filter;
...
public class FilterUtils {
    ...
    public  final String getProfileId(){
        RequestContext ctx = RequestContext.getCurrentContext();
        if (ctx.getRequest().getHeader(PROFILE_ID) !=null) {
            return ctx.getRequest().getHeader(PROFILE_ID);
        }else{
            return ctx.getZuulRequestHeaders().get(PROFILE_ID);
        }
    }

    public void setProfileId(String orgId){
        RequestContext ctx = RequestContext.getCurrentContext();
        ctx.addZuulRequestHeader(PROFILE_ID, orgId);
    }
